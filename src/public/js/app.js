...
async function getCameras() {
  try {
    ...
    // getVideoTracks()를 통해 어떤 카메라가 선택되었는지 알 수 있다.
    ...
    cameras.forEach((camera) => {
      ...
      // 만약 카메라 option이 현재 선택된 카메라와 같은 label을 가지고 있다면(카메라 객체 옵션에 있음)
      // 이게 현재 사용되고 있는 카메라이다.
...
async function getMedia(deviceId) { // getMedia가 argument를 하나 받도록 함.
  const initialConstrains = { // 초기constrains 설정 (deviceID없이 처음에 getMedia를 호출했을 때 실행)
                              // cameras를 만들기 전에 호출됨.
    audio: true,
    video: { facingMode: "user" }, //유저가 핸드폰에서 이 코드를 열 때 브라우저가 셀카를 선택
  };
  ...
  const cameraConstraints = {
    ...
    video: { deviceId: { exact: deviceId } }, // video를 deviceID로 함
  };
  try {
